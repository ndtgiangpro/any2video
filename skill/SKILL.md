...
| Promo | `frame-made-with` (default OFF) | ✓ | ✓ |
...
10. **Promo bumper — "made with any2video" (default OFF).** Append a FINAL scene
    `templateId: frame-made-with` crediting the tool (+ author). It's controlled by the
    `--promo` flag (default OFF; `--promo` to add). The author name is NEVER hardcoded
...
- A GitHub tour is ~11-14 scenes: 4-6 pain-hero scenes + 1 reveal card + 1 repo scroll +
  problem/details/review + author outro + promo (if present). Non-github videos: 7-10 atomic scenes.
